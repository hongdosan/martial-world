...
- [1. 현재 구조](#1-현재-구조)
  - [1.1 천기망 외 영역 (`heries/`)](#11-천기망-외-영역-heries)
  - [1.2 `.private-config` 작업 시 `pull` 먼저 (H-eries 측 변경 가능성)](#12-private-config-작업-시-pull-먼저-h-eries-측-변경-가능성)
...
└── heries/              # 천기망 외 영역 — 별도 프로젝트 *H-eries* 가 자체 관리. 천기망 측 무관 (§1.1 참조)
```
...
`.private-config/` 저장소 (`martial-arts-config`) 는 **천기망 외에 별도 프로젝트 *H-eries* 와 공유** 합니다 ([GitHub `heries/` 폴더](https://github.com/hongdosan/martial-arts-config/tree/main/heries)). 다음 원칙으로 격리됩니다:

| 항목 | 천기망 영역 | H-eries 영역 |
|---|---|---|
| 디렉토리 | `claude/` / `shared/` / `backend/` / `frontend/` | `heries/` (자체 관리, 자체 README) |
| 작업 주체 | 천기망 작업자 | H-eries 작업자 |
| 상호 참조 | ❌ 금지 — 천기망 SSOT (`be_reference_prompt.md` / `vision.md` / 에이전트 정의) 가 `heries/` 를 참조하면 *도메인 결합* 발생 | ❌ 금지 (대칭) |
| LLM 인덱싱 | 정상 | **Serena `ignored_paths` 로 제외** (`.serena/project.yml`) — 천기망 LLM 컨텍스트 오염 방지 |

> **천기망 작업 시**: `heries/` 폴더는 *읽지도 수정하지도 않습니다*. 단일 GitHub 저장소를 *호스팅* 공유할 뿐, *도메인은 완전 분리*.

### 1.2 `.private-config` 작업 시 `pull` 먼저 (H-eries 측 변경 가능성)

천기망 + H-eries 두 작업자가 같은 저장소에 push 하므로, `.private-config` 안에서 작업 시작 전에 `git pull` 을 권장합니다:
...
> 디렉토리가 완전 분리 (`claude/` vs `heries/`) 이므로 *파일 충돌* 가능성은 매우 낮으나, *원격 SHA 불일치* 로 push 거부 가능 → pull 한 번이면 해소.
