...
> 다음 진화 트리거: **🥇 우선순위 1 = 기획 사이클** (도메인 설계 / 화면 설계 / 요구사항 정의서) — 사용자 명시 "초반 기획 매우 중요 / rough X / 애자일 진행". BE 도메인 모듈 (`<domain>` placeholder → 실제 도메인) + FE prototype 정합성 결정 + frontend/src/ prototype 분리 시점 결정. **🥈 BE 영역**: `backend/` 첫 구조 셋업 (settings.gradle / buildSrc / app / core — 도메인 모듈은 도메인 설계 후) / 도메인 설계 사이클 / 보안 정책 사이클 (Spring Security / JWT / OAuth2). **🟢 FE 후속** (기획 / BE 합류 후): `shared/api/http.ts` 도입 (BE OpenAPI 명세 후) / `shared/config/env.ts` 도입 (첫 환경 변수 시점) / 디자인 토큰 (`@theme` / `theme.extend` — 디자인 시스템 사이클) / prototype 코드 본격 정리 / `card.tsx` lint 6 warnings 해소 / `eslint-plugin-boundaries` (FSD lint) / pre-commit hook (`husky` + `lint-staged`) / Prettier. **🟪 메타 도구** (별도 트랙): `agent-fe-tester` 시범 생성 / 통합 코디네이터 `agent-reviewer.md` / claude code hook 자동화 (`vibe-coding-flow §4` 자동 강제) / `.github/` CI/CD 보강.
