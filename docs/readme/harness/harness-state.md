...
> 다음 진화 트리거: **🥇 우선순위 1 = 기획 사이클 후속 — Phase 1 (세계관 사전) DDD Strategic Design** — `docs/readme/planning/vision.md` 헌장 SSOT 화 완료 후속. 다음 결정 영역 (vision.md §6 Deferred 인용): (1) Phase 1 *Bounded Context / Ubiquitous Language / Aggregate* 식별 — Event Storming 1인 변형 / 현 prototype entities 7종 (character/level/art/faction/fortune/misc/title) 검증 재료 활용 (2) Phase 1 *Aggregate 행위 카탈로그* (명령 + 이벤트 + 불변식) — 사용자 명시 "행위 명세 정말 중요" (3) Phase 1 *BDD Given/When/Then* 시나리오 — BE BDD 컨벤션 직접 입력 (4) Phase 1 *화면 설계* (Claude Artifact 활용) (5) Phase 1 *Phase 1 plan* (`.private-config/shared/prompt/plan/planning-phase-1-vision/` 또는 후속 명명). **🥈 BE 영역**: `backend/` 첫 구조 셋업 (settings.gradle / buildSrc / app / core) — Phase 1 도메인 식별 후 첫 도메인 모듈 생성 / 보안 정책 사이클 (Spring Security / JWT / OAuth2). **🥉 인프라 영역** (Phase 2 진입 시): GPU 서버 / 셀프 호스팅 LLM 추론 엔진 (Ollama/vLLM/llama.cpp) / 강호 IP 파인튜닝. **🟢 FE 후속** (Phase 1 화면 설계 후): `shared/api/http.ts` 도입 (BE OpenAPI 명세 후) / `shared/config/env.ts` / 디자인 토큰 / prototype 코드 본격 정리 / `card.tsx` lint 6 warnings 해소 / `eslint-plugin-boundaries` / pre-commit hook / Prettier / PWA 최소 추가 (manifest + service worker — vision.md §4.4). **🟪 메타 도구** (별도 트랙): `agent-fe-tester` 시범 생성 / 통합 코디네이터 `agent-reviewer.md` / claude code hook 자동화 / `.github/` CI/CD 보강. **⚪ Deferred** (vision.md §6): 외부 IP 저작권 검토 (Phase 3 진입 전) / 모바일 정식 앱 (Phase 2 검증 후) / 게임 엔진 (Phase 4 진입 시) / 수익화 (범위 외).
