...
> 다음 진화 트리거: **FE 디렉토리 이전 + Vite 마이그레이션 사이클 단계 2** (Vite — 본 사이클 진행 중). 단계 2 commit 후 본 문서 *표준-코드 정합* 표에서 `Vite` ❌ → ✅ 갱신 예정. 단계 3 commit 후 Phase B Minor 7건 보강 (DoD/검토 포커스 SSOT § 인용 + 영향 범위 항목 + 아티팩트 동기화 DoD). `backend/` △ → ✅ 갱신은 *backend/ 첫 셋업* 별도 사이클. `agent-fe-tester` 시범 생성 / 도메인 설계 / 보안 정책 사이클은 별도 트랙.
