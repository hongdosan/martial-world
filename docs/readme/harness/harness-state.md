...
| 2026-05-16 | **`히리즈` 저장소 `.private-config` 공유 — 사전 방어 조치** — 사용자 명시 *"미래에 생길 때 처리하는 것보다 지금 미리 방지를 하는게 맞다"*. 천기망 + 히리즈 두 프로젝트가 `martial-arts-config` 단일 저장소를 공유하는 사실에 대해 3가지 사전 방어: (1) **Serena LLM 컨텍스트 격리** — `.serena/project.yml` `ignored_paths` 에 `.private-config/히리즈` / `heries` / `Heries` 3종 추가 (정확한 표기 모름 → 변형 모두 커버) (2) **작업 절차 — pull-first** — `private-config.md §1.2` 신규 (히리즈 측 변경 가능성 → `.private-config` 진입 시 `git pull` 권장) (3) **천기망 외 영역 명시** — `private-config.md §1` 저장소 구조 표 + `§1.1` 격리 원칙 표 (디렉토리 / 작업 주체 / 상호 참조 / LLM 인덱싱). CURRENT.md §"병행 트랙" *Deferred → 사전 방어 조치 ✅* 로 갱신 | 3파일 (`.serena/project.yml` + `docs/readme/private-config.md` + `docs/readme/handoff/CURRENT.md`) + 본 문서 | 사용자 명시 — *"방어는 발생 후 처리보다 사전 처리"*. 비용 작고 효과 명확 (LLM 컨텍스트 오염 / push 거부 / 영역 혼선 — 모두 1회성 박아두면 영구 방어). 천기망 SSOT 와 히리즈 영역 *도메인 결합 금지* 원칙 박힘 |
...
> 다음 진화 트리거: **🥇 우선순위 1 = 기획 사이클 Phase 3 — DDD Strategic Design 본격** — `vision.md` 헌장 + `requirements-phase-1.md` Phase 2 산출물 closure 완료. 다음 결정 영역 (handoff `2026-05-10-planning-phase-1-2-closure.md` §"다음 세션 진입 절차" 인용): (1) **Event Storming 1인 변형** — requirements-phase-1.md §3 FR 7영역 40+ 건을 *시간순 이벤트* 로 재배열 (2) **Aggregate 후보 식별** — 이벤트 묶음 → Aggregate 경계 (3) **Bounded Context 식별** — vision.md §4.1.4 BC 가설 6개 (BaseWorldview/CustomWorldview/User/Community/Feedback + Ranking Phase 2) 검증 (합침 / 분리 / 추가) (4) **Ubiquitous Language 사전** — 강호 용어 (경지/심법/문파/기연/칭호/...) 정의 + 동의어 금지 (5) **Aggregate 행위 카탈로그** — 명령 + 이벤트 + 불변식 (사용자 명시 "행위 명세 정말 중요") (6) **Subdomain 분류** — Core/Supporting/Generic (vision.md §5.3 가설 검증) (7) 산출물 = `docs/readme/planning/domain-phase-1.md` (vision.md §5.1.3 표 정합). **🥈 BE 영역**: `backend/` 첫 구조 셋업 (settings.gradle / buildSrc / app / core) — Phase 1 도메인 식별 후 첫 도메인 모듈 생성 / 보안 정책 사이클 (Spring Security / JWT / OAuth2). **🥉 인프라 영역** (Phase 2 진입 시): GPU 서버 / 셀프 호스팅 LLM 추론 엔진 (Ollama/vLLM/llama.cpp) / 강호 IP 파인튜닝. **🟢 FE 후속** (Phase 1 화면 설계 후): `shared/api/http.ts` 도입 (BE OpenAPI 명세 후) / `shared/config/env.ts` / 디자인 토큰 / prototype 코드 본격 정리 / `card.tsx` lint 6 warnings 해소 / `eslint-plugin-boundaries` / pre-commit hook / Prettier / PWA 최소 추가 (manifest + service worker — vision.md §4.4). **🟪 메타 도구** (별도 트랙): `agent-fe-tester` 시범 생성 / 통합 코디네이터 `agent-reviewer.md` / claude code hook 자동화 / `.github/` CI/CD 보강. **⚪ Deferred** (vision.md §6): 외부 IP 저작권 검토 (Phase 3 진입 전) / 모바일 정식 앱 (Phase 2 검증 후) / 게임 엔진 (Phase 4 진입 시) / 수익화 (범위 외).
