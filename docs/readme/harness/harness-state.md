...
| 2026-05-09 | **단계 2 (Vite 마이그레이션)** — `react-scripts` (CRA) → Vite 5.x + Vitest. Phase C audit Major 5건 일괄: `public/index.html` → `frontend/index.html` root 이동 + `%PUBLIC_URL%` 제거 + `<script type="module">` / `react-app-env.d.ts` → `vite-env.d.ts` (`vite/client` types) / `package.json` scripts (`dev`/`build`/`preview`/`test`) + `homepage` 제거 + `"type": "module"` + eslintConfig 의 `react-app/jest` 제거 / `setupTests.ts` 검증 (변경 X — vitest 가 jest-dom 호환) / `build/` → `dist/`. 동반 변경: `vite.config.ts` 신규 (`base: '/martial-arts/'`, react plugin, vitest 설정) / `tsconfig.node.json` 신규 / `tsconfig.json` `types: [vite/client, vitest/globals, @testing-library/jest-dom]` 추가 (`bundler` 는 ts4.9.5 미지원으로 `node` 유지 — ts5 업그레이드 별도 사이클) / `@types/node` v16 → v22 (vite peer 요구) / `.env`·`.env.example`·`.env.dev` prefix `REACT_APP_*` → `VITE_*` (4 변수) / `env-var-convention.md` 전면 갱신 (Vite 로딩 우선순위 / 키 네이밍 / 코드 패턴 `import.meta.env.VITE_*` / 백엔드 매핑 표) / `getting-started.md` `npm start` → `npm run dev` / `fe-architecture.md` §"현재 상태" Build Tool ✅ / `fe_reference_prompt.md §1·§12` ✅ / `CLAUDE.md` 변경 이력 / 본 문서 정합성 표 + 변경 이력 | 본 라운드 정정 13파일 (코드 0 + docs 5 + .private-config 3 + frontend 신규/갱신 5) — FE 코드 (`src/**`) 자체 변경 0건 | Phase C audit Major 5건 일괄 처리. `App.test.tsx` "learn react" 텍스트 검색 실패는 *기존 CRA 템플릿 잔재 결함* (Plan 02 §6 비범위 — App.tsx 정리 별도 사이클) |

| 2026-05-09 | **단계 3 (Phase B Minor 보강)** — Phase B audit Minor 7건 일괄: `fe_develop_prompt.md` (M1·M4·M5·M7) + `fe_improvement_prompt.md` (M2·M4·M6) + `fe_review_prompt.md` (M3). DoD/검토 포커스 모두 SSOT § 인용 (BE 22/22 패턴 정합) + DoD 운영 헤더 (Synchronous Update 인용) + 영향 범위 4 → 8 항목 (라이브러리 어댑터 / OpenAPI / 환경변수 / 아티팩트) + 아티팩트 동기화 DoD (`agent-frontend.md §9` 인용). `fe_reference_prompt.md §12` / 본 문서 / `CLAUDE.md` 변경 이력 누적 | 본 라운드 정정 6파일 (.private-config 4 + docs 2). FE 코드 (`frontend/src/**`) 자체 변경 0건 (템플릿 정정만) | Phase B audit Minor 7건 일괄 처리 (선행 핸드오프 `2026-05-07-be-fe-consistency-cycle.md` Open Work §3). BE 깊이 (`be_develop_prompt.md` 22 DoD + 9 영향 범위) 와 대칭. **본 commit 으로 FE 디렉토리 이전 + Vite 마이그레이션 사이클 (1+2+3) 전체 종료** |

> 다음 진화 트리거: **사이클 closure 핸드오프** (`docs/readme/handoff/2026-05-08-fe-vite-migration-cycle.md`) 작성 + 일시정지 핸드오프 (`2026-05-08-fe-vite-migration-step1-pause.md`) 삭제. 그 후: **메타 정책 사이클 (Task #8)** — 자동 트리거 정책 (특정 양 누적 시 변경 이력 자동 압축) 결정. `backend/` △ → ✅ 는 *backend/ 첫 셋업* 별도 사이클. `agent-fe-tester` 시범 생성 / 도메인 설계 / 보안 정책 / `App.tsx` CRA 템플릿 정리 / TypeScript v5 업그레이드 / ESLint 재설정 / TanStack Query / Tailwind CSS 도입은 별도 트랙.
